...
class ToolIsolation(AppTool, Gerber):
		...
		for tool_sorted in sorted_tools:
			for tooluid_key, tooluid_value in self.iso_tools.items():
				...
				if truncated_dia == tool_sorted:
					...
					# Diameter
					if tooluid_value['tool_type'] == 'V':
						try:
							truncated_dia = f'{truncated_dia} {tooluid_value["data"]["vtipangle"]:.0f}° z={tooluid_value["data"]["cutz"]}'
						except:
							pass
	...
	def on_iso_tool_add_from_db_executed(self, tool: dict):
		"""
	...
	def on_tool_from_db_inserted(self, tool: dict):
		"""
...
class IsoUI:
	...
	def __init__(self, layout, app):
		...
		# 4th column is reserved (and hidden) for the tool ID
